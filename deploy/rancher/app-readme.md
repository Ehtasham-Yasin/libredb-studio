...
* AI-assisted querying
  * An optional AI assistant (bring your own key: Gemini, OpenAI, or a local model) explains a query in plain English from the engine's own EXPLAIN plan, on PostgreSQL, MySQL, SQLite, libSQL, DuckDB, Couchbase, ClickHouse, Apache Druid and Apache Trino. It also runs a read-only investigation agent on PostgreSQL, SQLite, DuckDB and SQL Server where the database, not the IDE, enforces the read-only session. It stays off unless you configure a provider.
* Self-hosted data
